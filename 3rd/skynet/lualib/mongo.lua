...
local function mongo_auth(mongoc)
	...
	return function()
		...
		local conf_replicaset = mongoc.replicaset
		local conf_backup = mongoc.backup
        skynet.error("mongo conf_replicaset:", conf_replicaset, " conf_backup:", tostring(conf_backup))
        local backup = {}
        local uniq_addrs = {}
        if conf_replicaset and conf_backup then
            for _, item in pairs(conf_backup) do
                local str = item.host .. ":" .. item.port
                if not uniq_addrs[str] then
                    uniq_addrs[str] = true
                    table.insert(backup, {host = item.host, port = item.port})
                end
            end
            if #backup > 0 then
        ...
        skynet.error("mongo ismaster rs_data:", tostring(rs_data))
		if rs_data.ok == 1 then
			if rs_data.hosts then
                ...
                skynet.error("mongo new backup:", tostring(backup))
				mongoc.__sock:changebackup(backup)
			end
			...
			elseif rs_data.primary then
                ...
                skynet.error("mongo no primary return and me:", tostring(rs_data.me))
			end
		end
	end
end
