...
local clusterd
local cluster = {}
local sender = {} -- key: nodename(配置表中给出的节点名), value: bee_clustersender服务的地址
local task_queue = {} -- 
...
function cluster.call(node, address, ...)
    -- skynet.pack(...) will free by beecluster.core.packrequest
    local s = sender[node]
    if not s then
...
skynet.init(function ()
    clusterd = skynet.uniqueservice("bee_clusterd")
end)
