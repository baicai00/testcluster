...
local cluster = require "beecluster.core"

local config_name = skynet.getenv "cluster"
local node_address = {} -- 保存cluster配置中的节点信息，例如节点配置: node1='127.0.0.1:8010',则通过loadconfig函数解析后存到node_address中，key为节点名称(即node1)，value是节点的地址信息
local node_sender = {} -- key:与node_address的key一样，value:clusersender服务的地址
local node_sender_closed = {} -- 保存被下线的节点(在cluster配置中，将节点的值改为false表示将节点下线，例如:node1=false) key:与node_address的key一样，value:true
local command = {}
local config = {}
local nodename = cluster.nodename() -- 返回由"hostname"+pid组成的字符串，如果获取hostname失败，则返回"noname"+pid
...
local function open_channel(t, key)
    ...
    if address == nil and not config.nowaiting then -- 配置中还没有节点的地址，而且配置中的nowaiting标志不是true，那么协程会阻塞直到节点的地址被设置
        local co = coroutine.running()
    ...
    if address then
        ...
        c = node_sender[key]
        if c == nil then
            c = skynet.newservice("bee_clustersender", key, nodename, host, port)
...
local node_channel = setmetatable({}, { __index = open_channel }) -- 貌似与node_sender结构一样???
...
local function loadconfig(tmp)
    if tmp == nil then
        ...
        if config_name then
            ...
            f:close()
            assert(load(source, "@"..config_name, "t", tmp))() -- 这里会把配置都添加到tmp表中,例如配置中有"test = 100",那么通过tmp.test可以访问到test配置项
        end
    end
    local reload = {}
    for name,address in pairs(tmp) do
        if name:sub(1, 2) == "__" then -- string.sub 返回字符串的子串， 该子串从 i 开始到 j 为止
            name = name:sub(3)
            config[name] = address
...
function command.proxy(source, node, name)
    ...
    if p == nil then
        p = skynet.newservice("bee_clusterproxy", node, name)
        -- double check
...
function command.register(source, name, addr)
    ...
    skynet.ret(nil)
    beelog_info(string.format("Register [%s] :%08x", name, addr))
end
...
function command.unregister(_, name)
    ...
    clearnamecache()
    skynet.ret(nil)
    beelog_info(string.format("Unregister [%s] :%08x", name, addr))
end
...
function command.socket(source, subcmd, fd, msg)
    if subcmd == "open" then
        beelog_info(string.format("socket accept from %s", msg))
        -- new cluster agent
        cluster_agent[fd] = false
        local agent = skynet.newservice("bee_clusteragent", skynet.self(), source, fd)
        ...
        if subcmd == "close" or subcmd == "error" then
            ...
            beelog_info(string.format("socket %s %d %s", subcmd, fd, msg or ""))
        end
    end
end
